...
@author: 
    ...
    Kieran Marray, St Catherine's College, University of Oxford
    Working at the Programme on Complexity Economics, the Institute for New
...
some set of univariate data. It imports a set of powerful distribution-fitting 
...
This was originally written to calculate the degree distribution of a network.
"""
...
#imports the necessary R packages into Python
#these are two packages which allow you to fit distributions

#Note that R stores data as vectors - so any data in the interface needs to be
#stored as a vector too



"""
Function to import a package from R into the python environment.
Note that if this is running in Conda, it uses the packages which have been
installed into the Conda environment NOT the packages which are installed on
the computer. So before you import a package using this, you need to install it
onto the Conda environment using the Anaconda Prompt and writing 'conda install
r-packagename. Then you can run this code to import it into this environment.

Note that if the package has not been uploaded to Conda beforehand you can create
it yourself. You do this as follows. Locate the R directory the package is installed 
as (normally 'cran'). Open Anaconda Prompt. Now write the following code 'conda
skeleton <directoryname> <packagename> (e.g conda skeleton cran goft). This builds
a skeleton for the package. Next write conda build --R=<version of R you have> 
r-<packagename> (e.g conda build --R=3.5.1 r-goft). This builds the package. Then
upload it to your anaconda account. To do this, type 'anaconda upload <path_to_the
package_you_built> -u <your_anaconda_cloud_accountname>. It should have given you
the pathway to the package when the previous code ran. This should upload the 
package to your conda cloud account. Then you have to download it again. You do this
by typing the final line of code 'conda install -c <your_anaconda_cloud_accountname>
r-<packagename>.
"""
...
#It does so using the 'goft' package in R, which performs goodness of fit tests.
#It starts with some more standard distributions and then iterates through to some
#more exotic distributions. If the p-values are below the level of the
#decision rule that you specify it then fits the distribution using the fitdistrplus
#package.
...
#Note the following thing about the pandas to r conversion pandas2ri.
#It automatically initialises the pandas dataframe as a R data object IN THE PYTHON
#ENVIRONMENT. It does not do so in the R environment. So if you are importing a 
#R function into the Python environment to then use it there, you can just proceed
#as normal. However if you are using robjects.r to directly write R code in the Python
#IDE, then you need to create a copy of the dataframe in the R global environment.
#The line of code below does this.
    
    #feeds the dataset into the R environment
    
    robjects.globalenv['A'] = A
    
    #This function now tests whether the data can be represented by a set of
    #distributions in turn
    
    #For each distribution, it first carries out the function in R. It then selects
